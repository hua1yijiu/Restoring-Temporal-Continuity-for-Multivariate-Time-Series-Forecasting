...
from layers.MoENetwork import Network
...
class Model(nn.Module):
    ...
    def forward(self, x, batch_x_mark=None, flag=False, **kwargs):
        # x: [Batch, Input, Channel]

        # if (flag):
        #     x = self.patch_aug(x)
        # B, L, C = x.shape
        # assert C == 7, f"[B,L,C] expected C={7}, got {x.shape}"
        # Normalization
        if self.revin:
            x = self.revin_layer(x, 'norm')

        if self.ma_type == 'reg':   # If no decomposition, directly pass the input to the network
            x = self.net(x, x, batch_x_mark, flag)
